...
# ① 단일 URL 예측 함수
def predict_url(url: str, model=xgb_model, thr: float = 0.5) -> None:
    """URL 하나를 입력받아 악성/정상 판정과 확률을 바로 출력"""
    feats = extract_url_features(str(url))
    row   = pd.DataFrame([feats])
    prob  = model.predict_proba(row)[0, 1]
    pred  = int(prob > thr)          # threshold 이상이면 1(악성)
    label = "악성(피싱)" if pred else " 정상"
    print(f"\nURL: {url}\n피싱 확률: {prob:.3f}  →  {label}")

# ------------------------------------------------------------
# ② 반복 입력 루프 (엔터만 치면 종료)d
print("\nURL을 입력해 보세요. (빈 줄 → 종료)")
while True:
    u = input("검사할 URL: ").strip()
    if not u:
        break
    predict_url(u, thr=0.6)     # 필요하면 임계값 조정 (예: 0.6)